...
public abstract class BrowserActivity extends ThemableBrowserActivity implements BrowserView, UIController, OnClickListener {
    ...
    // Static Layout
    @BindView(R2.id.drawer_layout) DrawerLayout mDrawerLayout;
    @BindView(R2.id.content_frame) FrameLayout mBrowserFrame;
    @BindView(R2.id.left_drawer) ViewGroup mDrawerLeft;
    @BindView(R2.id.right_drawer) ViewGroup mDrawerRight;
    @BindView(R2.id.ui_layout) ViewGroup mUiLayout;
    @BindView(R2.id.toolbar_layout) ViewGroup mToolbarLayout;
    @BindView(R2.id.progress_view) AnimatedProgressBar mProgressBar;
    @BindView(R2.id.search_bar) RelativeLayout mSearchBar;
...
//        mBrowserFrame.setLayoutTransition(new LayoutTransition());
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (i == android.R.id.home) {
            ...
            return true;
        } else if (i == R.id.action_add_to_homescreen) {
            if (currentView != null) {
            ...
            return true;
        } else if (i == R.id.action_new_tab) {
            newTab(null, true);
            return true;
        } else if (i == R.id.action_incognito) {
            ...
            return true;
        } else if (i == R.id.action_bookmarks) {
            openBookmarks();
            return true;
            ...
            openHistory();
            return true;
        } else if (i == R.id.action_downloads) {
            openDownloads();
            return true;
        } else if (i == R.id.action_add_bookmark) {
            if (currentUrl != null && !UrlUtils.isSpecialUrl(currentUrl)) {
                addBookmark(currentView.getTitle(), currentUrl);
            }
            return true;
        } else if (i == R.id.action_find) {
            findInPage();
            return true;
        } else if (i == R.id.action_reading_mode) {
            if (currentUrl != null) {
    ...
    public void onClick(View v) {
        ...
        } else if (i == R.id.button_quit) {
            currentTab.clearFindMatches();
            mSearchBar.setVisibility(View.GONE);

        } else if (i == R.id.action_reading) {
            Intent read = new Intent(this, ReadingActivity.class);
            read.putExtra(Constants.LOAD_READING_URL, currentTab.getUrl());
            startActivity(read);

        } else if (i == R.id.action_toggle_desktop) {
